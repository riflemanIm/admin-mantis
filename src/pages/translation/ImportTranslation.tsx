...
const ImportTranslation = (): JSX.Element => {
  ...
  const handleFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      filename != null &&
      validEndings.find((it) => filename.toLowerCase().endsWith(it))
    ) {
      ...
      sendNotification(
        `Название файла должно оканчиваться на ${validEndings.join(" или ")}`
      );
    }
    return null;
  };
        ...
        {!isLoadingFile ? (
          <>
            ...
            <FormHelperText color="warning">
              Будут удалены все переводы для выбранного проекта для всех языков.
              Будьте внимательны
            </FormHelperText>
