...
const ConfirmationCodeSettingsList = (): JSX.Element => {
  ...
  const processRowUpdate = React.useCallback(
    async (
      newRow: ConfirmationCodeSettingDto,
      oldRow: ConfirmationCodeSettingDto
    ) => {
      if (JSON.stringify(newRow) === JSON.stringify(oldRow)) return oldRow;
      const success = await actions.doUpdate(
        "/confirmationCode",
        newRow.confirmationCodeType,
        newRow
      )(dispatch);
      if (success)
    ...
    const result: GridColDef[] = [
      {
        ...
        align: "left",
        headerName:
          t("CONFIRMATIONCODESETTING.FIELDS.confirmationCodeType") ?? "",
      ...
      result.push({
        ...
        getActions: (params: GridRowParams) => [
          <GridActionsCellItem
            ...
            color="primary"
            onClick={() =>
              navigate(`/app/confirmationCodeSetting/${params.id}/records`)
            }
          />,
        ],
      });
