...
import {
  ...
  getValue,
  renderInputCell,
} from "../../components/SettingsGrid/utils";
...
const MedicalNetSettingsList = (): JSX.Element => {
  ...
  const processRowUpdate = React.useCallback(
    async (newRow: MedicalNetSettingDto, oldRow: MedicalNetSettingDto) => {
      if (getValue(newRow) === getValue(oldRow)) return oldRow;
      const success = await actions.doUpdate(
        "/medicalNetSetting",
        newRow.medicalNetSettingsId,
        { ...newRow, value: getStrValue(newRow) }
      )(dispatch);
      if (success)
