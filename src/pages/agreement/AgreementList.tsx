...
const AgreementList = (): JSX.Element => {
  ...
  const [agreement, setAgreement] = React.useState<string | undefined>(
    undefined
  );
  ...
  const columns: GridColDef<AgreementDto>[] = [
    ...
    },
    {
      ...
      width: 150,
      type: "singleSelect",
      valueOptions: agreementValue.agreementTypes.map(
        (it) => it.agreementTypeId
      ),
      valueFormatter: (value: number) =>
        agreementValue.agreementTypes.find((it) => it.agreementTypeId === value)
          ?.code ?? "",
    },
    {
      field: "platform",
  ...
  return (
    <Stack spacing={3}>
      ...
      <AgreementDialog
        isOpen={!!agreement}
        agreement={agreement}
        onClose={() => setAgreement(undefined)}
      />
